package com.github.fierioziy.particlenativeapi.core.mocks.nms.v1_18;

import com.github.fierioziy.particlenativeapi.core.mocks.nms.v1_13.Particle;
import com.github.fierioziy.particlenativeapi.core.mocks.nms.v1_13.ParticleType;
import com.github.fierioziy.particlenativeapi.core.mocks.nms.v1_17.Particles_1_17;

public class Particles_1_18 {

    public static ParticleType HEART = new ParticleType();
    public static Particle<?> FALLING_DUST = Particles_1_17.FALLING_DUST;
    public static Particle<?> BLOCK = Particles_1_17.BLOCK;
    public static ParticleType ENTITY_EFFECT = Particles_1_17.ENTITY_EFFECT;
    public static Particle<?> DUST = Particles_1_17.DUST;
    public static Particle<?> DUST_COLOR_TRANSITION = Particles_1_17.DUST_COLOR_TRANSITION;
    public static Particle<?> VIBRATION = Particles_1_17.VIBRATION;
    public static Particle<?> ITEM = Particles_1_17.ITEM;
    public static ParticleType FLAME = Particles_1_17.FLAME;
    public static ParticleType NOTE = Particles_1_17.NOTE;
    public static Particle<?> BLOCK_MARKER = new ParticleType();
    public static ParticleType DRAGON_BREATH = Particles_1_17.DRAGON_BREATH;
    public static ParticleType EFFECT = Particles_1_17.EFFECT;

}
